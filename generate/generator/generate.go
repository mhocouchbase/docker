...
const (
        ...
        PlatformAmznlinux = Platform("amazonlinux")
	...
	for _, ver := range versions {
                ...
                if constraints.Check(v1) && product == ProductServer {
                        for _, platform := range platforms {
                                dockerfile := path.Join(dir, ver, string(platform), "Dockerfile")
...
func (variant DockerfileVariant) dockerBaseImage() string {
	...
	case ProductServer:
                if variant.Platform == PlatformAmznlinux {
...
func (variant DockerfileVariant) pkgCommand() string {
        ...
        //only amzn and ubuntu are the valid platforms at the moment
        log.Printf("Platform: %v", variant.Platform)
        if variant.Platform == PlatformAmznlinux {
                return "yum"
        }
        return "apt-get"
...
func (variant DockerfileVariant) extraDependencies() string {
	if variant.Product == "couchbase-server" {
                if variant.Platform == PlatformUbuntu {
